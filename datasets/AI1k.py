...
class ImageRatingsDataset(Dataset):
    ...
    def __init__(self, root_dir, mode='train', suffix='', transform=None):
        ...
        # self.images_frame = pd.read_csv(os.path.join(root_dir, f'{mode}_image_ava{suffix}.csv'), sep=',')
        self.root_dir = root_dir
        self.transform = transform
        self.similarity = np.load(os.path.join(root_dir, f'{mode}_similarity{suffix}.npy'))
        print(f"self.similarity.shape: {self.similarity[0].shape}")
        # self.similarity = np.load(os.path.join(root_dir, f'{mode}_similarity_ava{suffix}.npy'))
        self.clip_image_features = np.load(os.path.join(root_dir, f'{mode}_clip_image_features{suffix}.npy'))
        #self.clip_image_features1 = np.load(os.path.join(root_dir, f'{mode}_hps_image_features{suffix}.npy'))
        #self.clip_image_features2 = np.load(os.path.join(root_dir, f'{mode}_alt_image_features{suffix}.npy'))
        self.clip_text_features = np.load(os.path.join(root_dir, f'{mode}_clip_text_features{suffix}.npy'))
        # Using BRISQUE
        #self.brisque_features = np.load(os.path.join(root_dir, f'{mode}_brisque_features_16{suffix}.npy'))

        # ava_features_path = os.path.join(root_dir, 'ava_features_0.npy')
        # if os.path.exists(ava_features_path):
        #     self.ava_features = np.load(ava_features_path)
        # else:
        #     self.ava_features = None
    ...
    def __getitem__(self, idx):
        ...
        sample = {
            ...
            #'brisque_features': self.brisque_features[idx],
        }
        if self.transform:
...
def AI1k_load_data(mode='train', split=0):
    ...
    data_dir = os.path.join('test_AI1k/')

    output_size = (224, 224) #tuple
    ...
    if mode == 'train':
        ...
        dataloader = DataLoader(transformed_dataset_train, batch_size=16,
        ...
        dataloader = DataLoader(transformed_dataset_valid, batch_size=16,
                                shuffle=False, num_workers=0, collate_fn=my_collate)
        ...
        dataloader = DataLoader(transformed_dataset_all, batch_size=16,
                                shuffle=False, num_workers=0, collate_fn=my_collate)
    else:
