...
def finetune_model(FusionModel, fetch_example, exp_name, num_epochs = 100, num_splits = 5, data_dir = os.path.join('AI-1k')):
    plcc_list = []
    srocc_list = []
    mse_list = []
    ...
    for split in range(num_splits):
        ...
        with torch.cuda.device(0):
            ...
            for epoch in range(num_epochs):
                ...
                train_spearman, train_pearson, train_mse = compute_metrics(dataloader_train_eval, model, epoch, split, "train", exp_name, criterion, fetch_example)
                ...
                val_spearman, val_pearson, val_mse = compute_metrics(dataloader_valid, model, epoch, split, "val", exp_name, criterion, fetch_example)
    ...
    # print(f'Average PLCC: {np.mean(plcc_list):.4f}{np.std(plcc_list):.3f}')
    print(f"Average PLCC: {np.mean(plcc_list):.4f}({np.std(plcc_list):.3f})")
    # print(f'Average SROCC: {np.mean(srocc_list):.4f}:{np.std(srocc_list):.3f}')
    print(f"Average SROCC: {np.mean(srocc_list):.4f}({np.std(srocc_list):.3f})")
    # print(f'Average MSE: {np.mean(mse_list):.4f}: {np.std(mse_list):.3f}')
    print(f"Average MSE: {np.mean(mse_list):.4f}({np.std(mse_list):.3f})")
